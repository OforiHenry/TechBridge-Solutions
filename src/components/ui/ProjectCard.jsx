...
export default function ProjectCard({ project, index }) {
  const { title, service, category, description, techStack, color, url, coverImage } = project;
    ...
    <>
      ...
      <div
        className={`relative h-52 overflow-hidden ${
          coverImage
            ? 'bg-primary'
            : `bg-gradient-to-br ${gradients[index % gradients.length]} flex items-center justify-center`
        }`}
      >
        {coverImage ? (
          <>
        ...
        <div className="absolute inset-0 bg-primary/80 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300">
          ...
          >
            {url ? 'View Live Site →' : 'View Details →'}
          </motion.span>
        </div>
  ...
  const cardClasses =
    'group glass-card rounded-2xl overflow-hidden cursor-pointer block';
